...
-(id)initWithNameSpace:(NSString*)nameSpace type:(IDPStorageType)type
{
    ...
    if(self)
    {
        ...
        if (type == IDPStorageDisk) {
            self.storageInnerEngine = [[[IDPStorageFileInner alloc] initWithNameSpace:nameSpace] autorelease];
        }
        else {
            self.storageInnerEngine = [[[IDPStorageSqliteInner alloc] initWithNameSpace:nameSpace] autorelease];
        }
    }
...
{
    dispatch_async(get_idp_diskio_queue(), ^{
        NSMutableData *saveData = [[[NSMutableData alloc] initWithBytes:&type length:sizeof(type)] autorelease];
        [saveData appendData:data];
